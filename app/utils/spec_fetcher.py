...
import ipaddress
import socket
from urllib import error, parse, request as _urllib_request
...
def fetch_spec_from_url(url: str) -> str:
    ...
    if _is_private_host(parsed.hostname or ""):
        raise ValueError(
            "URL must point to a public server. Private and internal addresses are not allowed."
        )
    ...
    try:
        ...
        if isinstance(reason, (TimeoutError, socket.timeout)):
            raise ValueError(
                "Could not reach URL - the request timed out. Check the address or your connection."
            ) from exc
        raise ValueError(
            "Could not reach URL - check the address or your connection."
        ) from exc
    except OSError as exc:
        # socket.timeout may be raised directly in some urllib internals
        raise ValueError(
            "Could not reach URL - check the address or your connection."
        ) from exc
    ...
    if len(body) > MAX_RESPONSE_BYTES:
        raise ValueError(
            "The spec URL returned a response that is too large to process (limit: 10 MB)."
        )
...
def _is_private_host(hostname: str) -> bool:
    ...
    try:
        ...
        return False  # non-IP hostname — DNS resolution not performed here
