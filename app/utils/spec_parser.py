...
def _validate_structure(spec: dict) -> None:
    ...
    try:
        ...
        # Use first line only — full error may include raw spec content (NFR6-NFR8)
        first_line = (
...
def _extract_parameters(path_params: list, op_params: list, spec: dict) -> list:
    ...
    for p in op_params:
        ...
        r = _resolve_one(p)
        if r:
            merged[(r.get("name"), r.get("in"))] = r  # operation overrides path
...
def _extract_auth(spec: dict) -> dict:
    ...
    Supports: http (bearer/basic), apiKey, oauth2, openIdConnect.
    oauth2 and openIdConnect return a typed stub — full support in Story 4.1.
    """
