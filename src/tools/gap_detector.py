...
Canonical location: src.tools.gap_detector
(Migrated from app/utils/spec_gap_detector.py — renamed, no logic changes.)
        ...
        if _has_missing_success_response(endpoint):
            gaps.append(
                _gap_record(
                    ...
                    (
                        f"Endpoint {endpoint_key} has no defined success"
                        " response schema - what does a successful response return?"
                    ),
                    "text_area",
                )
            )
        ...
        if _has_auth_ambiguity(spec, raw_operation):
            gaps.append(
                _gap_record(
                    ...
                    "auth.type",
                    (
                        f"Endpoint {endpoint_key} has unclear auth requirements"
                        " - which auth type should Sata use?"
                    ),
                    "select",
                    AUTH_OPTIONS,
                )
